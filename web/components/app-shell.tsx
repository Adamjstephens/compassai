...
export function makeMissedOpportunityReport(results: JobResult[], mirrorLeads: MirrorLead[]) {
  ...
  const agentSections = Array.from(grouped.entries()).map(([agent, calls]) => {
    ...
    const callSections = calls.map((result) => {
      ...
      const handled = (analysis.handledObjections ?? []).map((item) => `<section class="handled"><div class="finding-head"><div><span>${escapeHtml(item.category.replaceAll("_", " "))}</span><h4>${escapeHtml(item.title)}</h4></div><strong>${Math.round(item.confidence * 100)}% confidence</strong></div><p>${escapeHtml(item.assessment)}</p><div class="evidence-grid"><div><span>Customer objection${item.customerTrigger.startTime === undefined ? "" : ` · ${escapeHtml(reportTime(item.customerTrigger.startTime))}`}</span><blockquote>${escapeHtml(item.customerTrigger.text)}</blockquote></div><div><span>Effective agent response${item.agentResponse.startTime === undefined ? "" : ` · ${escapeHtml(reportTime(item.agentResponse.startTime))}`}</span><blockquote>${escapeHtml(item.agentResponse.text)}</blockquote></div></div><div class="coaching"><strong>Technique used</strong><p>${escapeHtml(item.technique)}</p></div></section>`).join("");
      return `<article class="call-report"><header class="call-header"><span>Individual call review</span><h3>${escapeHtml(titleFor(result, mirrorLeads))}</h3></header><div class="call-meta"><span>Agent: <strong>${escapeHtml(meta.agent || "Not detected")}</strong></span><span>Customer: <strong>${escapeHtml(meta.customer || "Not matched")}</strong></span><span>Phone: <strong>${escapeHtml(meta.phone ? formatPhone(meta.phone) : "Not matched")}</strong></span><span>Disposition: <strong>${escapeHtml(analysis.disposition.value.replaceAll("_", " "))}</strong></span><span>Call time: <strong>${escapeHtml(fmtSeconds(result.duration_seconds))}</strong></span><span>Analysis time: <strong>${escapeHtml(fmtSeconds(result.grading_seconds))}</strong></span>${meta.clover ? `<span>Clover: <a href="${escapeHtml(meta.clover)}">Open matched lead</a></span>` : ""}</div><h4>Missed opportunities</h4>${findings || `<div class="no-findings"><strong>No supported missed opportunities</strong><span>${escapeHtml(analysis.disposition.reason)}</span></div>`}<h4>Objections handled well</h4>${handled || `<div class="no-handled"><strong>No positively supported objection handling recorded</strong><span>Re-run this call with the current analysis version to evaluate handled objections.</span></div>`}<h4>Transcript</h4><div class="search-row"><input class="search" data-target="m${index}" placeholder="Search this transcript"><button type="button" data-prev="m${index}">Previous</button><button type="button" data-next="m${index}">Next</button><span data-count="m${index}">0 matches</span></div><pre class="transcript" id="m${index}">${escapeHtml(result.transcript_text)}</pre></article>`;
    }).join("");
  ...
  const handledTotal = analyzed.reduce((sum, result) => sum + (result.missed_opportunity_analysis?.handledObjections?.length ?? 0), 0);
...
function MissedOpportunityReviewPanel({
  ...
  const handledObjections = analysis?.handledObjections ?? [];
    ...
    <section className="review-workstation opportunity-workstation">
      ...
      <div className="review-main">
        ...
        <div className="review-summary opportunity-summary">
          ...
          <div><span>Handled well</span><strong>{analysis ? handledObjections.length : "–"}</strong></div>
